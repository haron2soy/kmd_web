...
export default function ShortRangeDiscussion() {
  const [document_path, setDocument] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const { headerRef } = useScrollToHeaderDoc(80, !loading);
  

  // Fetch Latest Short-range discussion document
  useEffect(() => {
    fetch("/api/forecasts/latest-doc/?slug=short-discussion")
      .then(res => {
        if (!res.ok) {
      ...
      .then(data => {
        if (data?.document) {
          // Django media URLs are already complete
      ...
      <PageLayout>
        <div className="container mx-auto px-4 py-10 md:py-12 lg:py-16 max-w-6xl">
          <div className="text-center py-20">
            ...
            <p className="text-gray-600">Loading Short Range Discussion...</p>
          </div>
        </div>
      </PageLayout>
    );
          ...
          <div className="lg:col-span-9">
            <header ref= {headerRef} className="mb-6 md:mb-12">
              ...
              <p className="text-lg text-gray-600 max-w-3xl">
                Monthly short range forecast discussion document.
              </p>
            </header>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              {error ? (
                ...
                <FileViewer
                  title="Short Range Discussion"
                  description="Monthly short range forecast discussion document"
                  fileUrl={document_path || '/document.doc'}
                  fileType="doc"
                />
              )}
            </div>
