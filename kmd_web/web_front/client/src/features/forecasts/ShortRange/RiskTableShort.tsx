...
export default function ShortRangeRiskTable() {
  const [document_path, setDocument] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const { headerRef } = useScrollToHeaderDoc(80, !loading);
  
  // Fetch Latest Short-range Risk table document
  useEffect(() => {
    fetch("/api/forecasts/latest-doc/?slug=short-risktable")
      .then(res => {
        if (!res.ok) {
                ...
                <div className="space-y-1">
                  {relatedLinks.map((link) => (
                    <SidebarLink
                      ...
                      href={link.href}
                      label={link.label}
                      //isActive={link.href === "/forecasts/risktable-short"} // Active for this page
                    />
                  ))}
                </div>
              </div>
