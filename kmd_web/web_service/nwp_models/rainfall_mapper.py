...
class RainfallMapper:
    ...
    def load_data(self):
        ...
        self.rain = rain_slice

    # ---------------------------
    # Create map (FULL GRID)
    # ---------------------------
    def generate_map(self):
        if self.rain is None or np.all(np.isnan(self.rain)):
            print(f"[RainMapper] No valid rainfall data at timestep. Skipping map.")
            return
        ...
        cmap = colormaps["turbo"].resampled(len(fixed_levels) - 1)
